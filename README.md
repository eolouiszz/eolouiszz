# Olá, eu sou Louis 👋

Sou desenvolvedor, entusiasta de tecnologia e **hacker ético em formação** desde cedo.  
Gosto de explorar programação, redes, segurança e design, criando projetos práticos e aprendendo todos os dias.

---

## 💻 Minhas Estatísticas

[![Top Langs](https://github-readme-stats-fast.vercel.app/api/top-langs?username=eolouiszz&layout=compact&theme=dracula)](https://github.com/eolouiszz)  
[![GitHub Stats](https://github-readme-stats-fast.vercel.app/api?username=eolouiszz&theme=dracula&show_icons=true)](https://github.com/eolouiszz)

---

## 🛠️ Tecnologias & Ferramentas

### Linguagens de Programação
- Python, JavaScript, Java, Lua
- Manipulação de JSON

### Frameworks e Bibliotecas
- React, Bootstrap, Flask

### Cybersegurança & Pentest
- Nmap, Wireshark, Burp Suite, Metasploit, Hydra, Nikto

### Ferramentas & Softwares
- Git / GitHub, VS Code, Roblox Studio, Blender, Photoshop
- Zsh, Bash / Shell

### Sistemas Operacionais
- Ubuntu (principal), experiência com Debian e distros baseadas em Debian

---

## 🚀 Bonone e Portfólio

- 🌐 [Meu Portfólio](https://louiszzz.vercel.app/) – Projetos, estudos e trabalhos recentes  
- 🎮 Bonone Project 🐢 – Futuro laboratório de jogos, assets e experimentos em Roblox  

> "Aprender é uma jornada contínua. Todo dia é dia de explorar algo novo."
